...
import {Container, Button, PlayerList, Modal} from '../../components'
...
class Queue extends React.Component {
  ...
  render() {
      ...
      const roomDetails = this.props.navigation.getParam('roomDetails');
      const userRoom = this.props.common.rooms.find(room => room.id === user.room.id);
    ...
    return userRoom ? (
      <Container>
          {this.state.showModal && <Modal onClose={() => this.setState({showModal:false})}><PlayerList players={listPlayers} /></Modal>}
          
        <Button onPress={() => this.playOffline()} title="Täytä huone boteilla" />
        ...
        <Text style={styles.text}>Pelaajia: {userRoom.players.length}/{roomDetails.playersAmount}</Text>
