...
class Game extends React.Component {
  ...
  myTurn(player){
    if(this.props.game.turn !== this.props.user.id || this.props.user.id !== player.id){
      return false
    }
    return true
  }
  ...
  changeCards(me){
      ...
      const name = this.type === 'paskahousu' ? 'PH change cards' : 'change cards'
  ...
  getPlayers(players){
    const myIndex = players.findIndex(player => player.id === this.props.user.id);
  ...
  createCenter(me){
    const {land, tikkiStarted, tikkiRoundWinner, pokerWinner, tikkiWinner, gameWinner, moneyExchange, players} = this.props.game;
    ...
    if(gameWinner && moneyExchange){
      return(
        <Text style={styles.text}>Pelin voitti {gameWinner.name}</Text>
      );
    }
    if(land && tikkiStarted){
      return(
        <Text style={styles.land(currentLand)}>{currentLand.icon}</Text>
        );
    }else if(tikkiRoundWinner && !land){
      return(
        <Text style={styles.text}>Tikkikierroksen  voitti {this.props.game.players.find(p => p.id === tikkiRoundWinner).name}</Text>
      );
    }
    return null
  }
...
createSelect(player){
  return(
      <ScrollView horizontal={true} style={{position:'absolute', bottom:'5%', width:'50%', zIndex:10, paddingLeft:20, height:'35%'}} >
          {player.cards.map((card, i) => (
          <Card card={card} i={i} select={true} key={card.id} onPress={() => this.onClickCard({player, card})}/>
          ))}
      </ScrollView>
  )
}
...
onClickCard({player, card}){
  ...
  render(){
    ...
    const Player = ({player, i}) => {
        switch(this.type){
            case 'paskahousu':
              return <TikkipokeriPlayer player={player} i={i} amount={players.length} />
          default:
              return <TikkipokeriPlayer player={player} i={i} amount={players.length} />
        }
    }
...
//     {table.length > 0 && <button onClick={() => this.myTurn(player) && this.takeTable(player)}>Nosta pöytä</button>}
              ...
              return (
                  <View style={styles.centerRow}>
                      <View style={{flex:0.25}}>
                        {
                          game.turn === me.id && (cardsSelected 
                          ? <Button title="Lyö" onPress={() => this.changeCards(me)} />
                          : !me.cardTaken && <Button title="Nosta pakasta" onPress={() => this.takeCard()} />
                          )}
                      </View>
                      <View style={{flex:0.25, alignItems:'center'}}>
                        {
                          game.firstTableCard && <Card card={game.firstTableCard} center={true}/>
                        }
                      </View>
                      <View style={{flex:0.25}}>
                        <View style={{flex:1, flexDirection:'row'}}>{(game.cards || []).map((c,i) => <Card card={c} i={i} hiddenCenter={true}/>)}</View>
                      </View>
                      <View style={{flex:0.25}}>
                        {
                          this.myTurn(me) 
                          && game.table.length > 0 && <Button title="Nosta pöytä" onPress={() => this.takeTable()} /> 
                        }
                      </View>
                  </View>
              )
    ...
    return(
      <Table>
        ...
        <TouchableOpacity style={styles.menuButton} onPress={() => this.setState({showMenu:!this.state.showMenu})}>
                {this.state.showMenu 
        ...
        {this.props.game.turn === this.props.user.id && this.createSelect(players[0])}
      {players.map((player, i) => <Player player={player} i={i} key={player.id} />)}
      </Table>
    );
  }
}
