...
class TikkipokeriPlayer extends React.Component{
  ...
  myTurn(){
    if(this.props.game.turn !== this.props.user.id || this.props.user.id !== this.props.player.id){
      return false
    }
    return true
  }
  ...
  render(){
    ...
    const styles = {
      ...
      info:{
        ...
        backgroundColor: this.props.game.turn === player.id ? '#2079d8' : 'rgba(0,0,0,0.5)'
      },
      name:{
        color:'white',
        // alignItems:'center',
  ...
  const visibleCards = player.shouldRevealHand ? player.hand.hand : player.cards
    return(
      <View style={styles.player}>
        ...
        <View style={styles.info}>
          ...
          {(!this.myTurn() || player.shouldRevealHand) && <View style={styles.tableCard}>
            {player.shouldRevealHand 
            ? <View>
                <Text style={styles.winner}>{player.hand.name}</Text>
                <Text style={styles.winner}>{this.getWinner(player)}</Text>
              </View>
            : player.firstTableCard &&  <Card card={player.firstTableCard} table={true}/>
            }
          </View>}

          {(!this.myTurn() || player.shouldRevealHand) && <View style={styles.cards}>
            {visibleCards.map(card => 
              <Card
                key={card.id} 
                hidden={player.shouldRevealHand ? false : (this.isMe() ? false : true)} 
                card={card}
              />
            )}
          </View>}
