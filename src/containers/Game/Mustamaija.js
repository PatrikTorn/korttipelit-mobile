...
class Mustamaija extends React.Component {
  ...
  myTurn(player){
    if(this.props.game.turn.id !== this.props.user.id || this.props.user.id !== player.id){
      return false
    }
    return true
  }
  ...
  trashCards(cards){
      this.props.socket.emit('MM trash card', {table:this.state.selectedTableCard, hand:null}) //TODo
  }
  ...
  createCards(player){
      return(
        <div className="cards" style={{width:player.table.length === 0 && '90%'}}>
      ...
      <Table>
        {players.map((player, i) => (
          <div
            key={Math.random()}
            className="player"
            id={`p${i+1}-${(players.length % 2 === 1) ? players.length + 1 : players.length}`}
          >
          {this.createCards(player)}
            <div className="changeCards">
            
            {landCard && <Card data={landCard} visible={true} onClick={() => console.log("ASD")}/>}
            {table.map((card, i) => <div style={{marginLeft:40*i, border:this.state.selectedTableCard.id === card.id && '4px solid red'}}><Card data={card} visible={true} key={card.id} onClick={() => card.enabled && this.selectTableCard(card)}/></div>)}
