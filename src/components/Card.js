...
export const Card = ({card, onPress, hidden, table, select, i, center, hiddenCenter}) => {
    ...
    if(hidden) return <Image source={cardHidden} style={styles.cardHidden}></Image>
    if(table) return <Image source={cardImage} style={styles.tableCard}></Image>
    if(center) return <Image source={cardImage} style={styles.cardCenter}></Image>
    if(hiddenCenter) return <Image source={cardHidden} style={styles.cardCenterHidden(i)}></Image>
...
const styles = {
      ...
      cardSelectContainer: (card, i) => ({
        marginLeft:i === 0 ? 0 : -30, width:'auto', 
        aspectRatio:200/306,
